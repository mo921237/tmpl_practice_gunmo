...
function getLiTag($text){
	return "<li>".$text."</li>";
}

function removeTags($text){
	$text->children(1)->remove();
	$text->children(0)->remove();
	return getLiTag($text);
}
// Write found domains string to specified output file
$html = file_get_html('http://m.coinreaders.com/b.html');
$ret = '<html><head>'
.'<meta name="viewport" content="width=device-width, user-scalable=0, initial-scale=1, minimum-scale=1, maximum-scale=1">'
.'<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>'
.'</head><body>'
.'<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css">'
.'<link rel="stylesheet" type="text/css" href="./top_news.css" />'
.'<script type="text/javascript">'
.'	var links = ["http://m.coinreaders.com","https://www.decenter.kr","https://m.zdnet.co.kr"];'
.'	$(document).ready(function(){'
.'		$("#MBody ul li a").on("click",function(e){'
.'			var idx = $(this).parent().parent().attr("id").substr(3);'
//.'		location.href = links[idx]+$(this).attr("href");'
.'			e.preventDefault();'
.'			e.stopPropagation();'
.'			console.log($(this).attr("href"));'
.'			parent.postMessage(links[idx]+$(this).attr("href"),"https://bananatok.imweb.me/");'
.'		});'
.'	});</script>';
$ret .= '<ul class="nav nav-tabs">'
.'<li class="nav-item"><a class="nav-link active show" data-toggle="tab" href="#tab0"><img src="./tab0.png"></a></li>'
.'<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#tab2"><img src="./tab2.png"></a></li>'
.'<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#tab1"><img src="./tab1.png"></a></li>'
.'</ul>';
$ret .= '<div id="MBody">';
//<a href="/a.html?uid=5113&amp;sc=&amp;sc2="><span class="main_title">글로벌 경제위기 경고음…비트코인, 금(金)처럼 안전자산 될 수 있을까</span></a>
$ret .= '<ul id="tab0" class="tab-pane tab-content show active">';
//$ret = $html->find('div[id=MBody]');
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',0));
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',1));
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',2));
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',3));
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',4));
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',5));
$ret .= removeTags($html->find('div[id=MBody] ul',0)->find('a',6));
//echo $ret;
//$ret->find('ul',0)->children(0)->remnove();
$ret .= '</ul>';
$html = file_get_html('https://www.decenter.kr/NewsList/GZ03');
$ret .= '<ul id="tab1" class="tab-pane tab-content">';
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(0)->find('dl dt',0)->children(0));
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(1)->find('dl dt',0)->children(0));
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(2)->find('dl dt',0)->children(0));
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(3)->find('dl dt',0)->children(0));
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(4)->find('dl dt',0)->children(0));
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(5)->find('dl dt',0)->children(0));
$ret .= getLiTag($html->find('div[id=m-left-scroll-in] ul',0)->children(6)->find('dl dt',0)->children(0));
$ret .= '</ul>';
$opts = [
    "http" => [
        "method" => "GET",
		...
		.'Cookie: PCID=15653430301495525387426; OAX=r36ro11NPTgAB2Wi; _ga=GA1.3.607444566.1565343033; _gid=GA1.3.1600979306.1565527865; ASPSESSIONIDQCSADQRS=BFAIANPDOPBMBNEDBNEBKHHL; __utmc=243917017; __utmz=243917017.1565528102.1.1.utmcsr=zdnet.co.kr|utmccn=(referral)|utmcmd=referral|utmcct=/; zdknum=0000; ASPSESSIONIDSEQBCRSS=GICCNGPDKJDBAKBKKJAIKDNB; __utma=243917017.607444566.1565343033.1565528102.1565572398.2; _pk_ref.zdnet_mobile.fecc=%5B%22%22%2C%22%22%2C1565572401%2C%22http%3A%2F%2Fwww.zdnet.co.kr%2F%22%5D; _pk_ses.zdnet_mobile.fecc=1; __utmb=243917017.2.10.1565572398; _pk_id.zdnet_mobile.fecc=ef90867522422218.1565528102.2.1565572412.1565572401.; dable_uid=67810615.1552528772794; _td=b9d1d148-94e7-4260-9677-e4164ab95e85; __gads=ID=1df8ca082fd1b5c8:T=1565572438:S=ALNI_MbX6uwE_P4PTd9olu0okIV7Rft9qg; ttx_t_r={"1091659477":{"criteo":"0"}}; ASPSESSIONIDCUTBAQQQ=DANOCNPDKFBMEEGOMDCPPABH; ASPSESSIONIDQGSADQRS=JJLPANPDOMOKCAADJIJBPCAD\r\n'
    ]
];
$context = stream_context_create($opts);
$html = file_get_html('https://m.zdnet.co.kr/news_list_ajax.asp?page=1', false, $context);
//$html = file_get_contents('https://m.zdnet.co.kr/news_list_ajax.asp?page=1', false, $opts);
$ret .= '<ul id="tab2" class="tab-pane tab-content">';
$ret .= getLiTag($html->find('a',1));
$ret .= getLiTag($html->find('a',3));
$ret .= getLiTag($html->find('a',5));
$ret .= getLiTag($html->find('a',7));
$ret .= getLiTag($html->find('a',9));
$ret .= getLiTag($html->find('a',11));
$ret .= getLiTag($html->find('a',13));
$ret .= '</ul>';
$ret .= '</div>';
$ret .= '</body>';
$ret .= '<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js"></script>';
$ret .= '</html>';

//file_put_contents('bananatok.php', $ret);
file_put_contents('/var/www/stillknow/portfolio/bananatok/bananatok.php', $ret);
