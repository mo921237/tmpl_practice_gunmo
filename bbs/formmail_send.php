...
//alert("{$name}, {$femail}, '{$to}', [{$name}] {$femail} from {$_SERVER['HTTP_HOST']}, $mail_content");
    try{
        if(mailer($name, $femail, $to, "[{$name}] {$femail} from {$_SERVER['HTTP_HOST']}", $mail_content, $type, $file)){
            die("00");
        } else {
            die("실패 하였습니다.");
        }
