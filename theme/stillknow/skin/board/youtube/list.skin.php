...
//유튜브를 목록에 출력하기
define('USE_LIST_YOUTUBE', true);



//게시판관리자 여분필드 1번에 열 수를 입력, 없으면 기본 3열
$gallery_col = 4;
if ($board['bo_1']) {
    $gallery_col = $board['bo_1'];
}
...
add_javascript('<script src="' . G5_THEME_URL . '/plugin/lazyload/jquery.lazyloadxt.min.js"></script>');
add_javascript('<script src="' . G5_THEME_URL . '/plugin/lazyload/jquery.lazyloadxt.video.min.js"></script>');
add_javascript('<script src="' . G5_THEME_URL . '/plugin/lazyload/jquery.lazyloadxt.extra.min.js"></script>');
...
<div class="youtube-list-wrap container">
    <!-- 게시판 목록 시작 { -->
    <div id="bo_list" style="width:<?php echo $width; ?>">
        ...
        <form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
            ...
            <div class="board-table-wrap">
                <div class="row img-hover-effect-gallery">
                    <?php
                    for ($i = 0; $i < count($list); $i++) {
                        if ($list[$i]['is_notice']) {
                            $grid = "col-sm-12 notice-grid";
                        } else {
                            $grid = "col-lg-{$gallery_col} col-md-6 col-sm-12";
                        }
                        ?>
                        <div class="gall-item <?php echo $grid ?>">
                            <?php if ($list[$i]['is_notice']) { ?>
                                ...
                                <div class="img-hover-item" id="youtube-list-view<?php echo $i ?>">
                                    <?php if ($list[$i]['is_notice']) { ?>
                                        ...
                                        if ($thumb['src']) {
                                            $img_content = '<img src="' . $thumb['src'] . '" alt="' . $thumb['alt'] . '" width="' . $board['bo_gallery_width'] . '" height="' . $board['bo_gallery_height'] . '" class="gall-photo" id="img-list-view' . $i . '">';
                                            $poster = $thumb['src'];
                                        } else {
                                            $img_content = '<img src="' . G5_THEME_IMG_URL . '/no-image.png" width="' . $board['bo_gallery_width'] . '" height="' . $board['bo_gallery_height'] . '" class="gall-photo" id="img-list-view' . $i . '">';
                                            $poster = '';
                                        }
                                    }

                                    if ((isset($list[$i]['wr_link1']) && $list[$i]['wr_link1'] != NULL) && USE_LIST_YOUTUBE == true) {
                                        ...
                                        <div class="plyr__video-embed" id="player_link<?php echo $i ?>">
                                            <iframe src="https://www.youtube.com/embed/<?php echo $youtube1 ?>?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" poster="<?php echo $poster ?>" allowfullscreen allowtransparency allow="autoplay"></iframe>
                                        </div>
                                        <?php
                                    } else {
                                ...
                                <div class="gall-info">
                                    ...
                                    <a href="<?php echo $list[$i]['href'] ?>">
                                        ...
                                        <?php if ($list[$i]['comment_cnt']) { ?><span class="sr-only">댓글</span>
                                            <span class="comment-count"><?php echo $list[$i]['comment_cnt']; ?></span>
                                ...
                                <ul class="write-info">
                                    ...
                                    <li>HIT:<?php echo $list[$i]['wr_hit'] ?></li>
                                    <?php if ($is_good) { ?>
                                        <li>
